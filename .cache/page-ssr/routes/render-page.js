...
exports.components={"component---src-pages-404-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-404-tsx */ 218).then(__webpack_require__.bind(__webpack_require__, 2513)),"component---src-pages-about-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-about-tsx */ 49).then(__webpack_require__.bind(__webpack_require__, 2766)),"component---src-pages-index-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-index-tsx */ 691).then(__webpack_require__.bind(__webpack_require__, 7200)),"component---src-pages-projects-tsx":()=>__webpack_require__.e(/* import() | component---src-pages-projects-tsx */ 317).then(__webpack_require__.bind(__webpack_require__, 7166))};
...
const trimmedPathname=stripPrefix(pathname,decodeURIComponent("/cerasamson"))// Remove any hashfragment
...
if(this.staticQueryDb[staticQueryHash]){const jsonPayload=this.staticQueryDb[staticQueryHash];return{staticQueryHash,jsonPayload};}return this.memoizedGet(`${"/cerasamson"}/page-data/sq/d/${staticQueryHash}.json`).then(req=>{const jsonPayload=JSON.parse(req.responseText);return{staticQueryHash,jsonPayload};}).catch(()=>{throw new Error(`We couldn't load "${"/cerasamson"}/page-data/sq/d/${staticQueryHash}.json"`);});})).then(staticQueryResults=>{const staticQueryResultsMap={};staticQueryResults.forEach(({staticQueryHash,jsonPayload})=>{staticQueryResultsMap[staticQueryHash]=jsonPayload;this.staticQueryDb[staticQueryHash]=jsonPayload;});return staticQueryResultsMap;});return Promise.all([componentChunkPromises,staticQueryBatchPromise]).then(([pageResources,staticQueryResults])=>{let payload;if(pageResources){payload={...pageResources,staticQueryResults};finalResult.payload=payload;emitter.emit(`onPostLoadPageResources`,{page:payload,pageResources:payload});}this.pageDb.set(pagePath,finalResult);if(finalResult.error){return{error:finalResult.error,status:finalResult.status};}return payload;})// when static-query fail to load we throw a better error
...
this.loadPageDataJson(pagePath));}}hovering(rawPath){this.loadPage(rawPath);}getResourceURLsForPathname(rawPath){const pagePath=findPath(rawPath);const page=this.pageDataDb.get(pagePath);if(page){const pageResources=toPageResources(page.payload);return[...createComponentUrls(pageResources.page.componentChunkName),createPageDataUrl(pagePath)];}else{return null;}}isPageNotFound(rawPath){const pagePath=findPath(rawPath);const page=this.pageDb.get(pagePath);return!page||page.notFound;}loadAppData(retries=0){return this.memoizedGet(`${"/cerasamson"}/page-data/app-data.json`).then(req=>{const{status,responseText}=req;let appData;if(status!==200&&retries<3){// Retry 3 times incase of non-200 responses
...
/******/ 			return "" + {"49":"component---src-pages-about-tsx","218":"component---src-pages-404-tsx","317":"component---src-pages-projects-tsx","691":"component---src-pages-index-tsx"}[chunkId] + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
await apiRunnerAsync(`replaceRenderer`,{bodyComponent,replaceBodyHTMLString,setHeadComponents,setHtmlAttributes,setBodyAttributes,setPreBodyComponents,setPostBodyComponents,setBodyProps,pathname:pagePath,pathPrefix:"/cerasamson"});// If no one stepped up, we'll handle it.
...
if(!isRedirect(e))throw e;}}apiRunner(`onRenderBody`,{setHeadComponents,setHtmlAttributes,setBodyAttributes,setPreBodyComponents,setPostBodyComponents,setBodyProps,pathname:pagePath,loadPageDataSync,bodyHtml,scripts,styles,pathPrefix:"/cerasamson"});// we want to run Head after onRenderBody, so Html and Body attributes
